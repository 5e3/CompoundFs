...
TEST(Path, noSubFolderDoesNotChangePath)
{
    ...
    ASSERT_EQ(p.m_parentFolder , Folder::Root);
...
TEST(Path, createCreatesSubFolders)
{
    ...
    ASSERT_TRUE(p.create(&ds));
    ASSERT_NE(p.m_parentFolder , Folder::Root);
...
TEST(Path, creationOfFolderFailsWhenTheNameIsUsedForAnAttribute)
{
    ...
    ASSERT_TRUE(p.create(&ds));
    ds.addAttribute(DirectoryKey(p.m_parentFolder, p.m_relativePath), 1.1);
    Path p2("folder/folder/attribute/test");
    Path p3 = p2;
    ASSERT_TRUE(!p2.create(&ds));
    ASSERT_EQ(p2 , p3);

    ds.remove(DirectoryKey(p.m_parentFolder, p.m_relativePath));
    ASSERT_TRUE(p2.create(&ds));
    ASSERT_NE(p2 , p3);
}
