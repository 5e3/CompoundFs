...
{
public:
    TestResult(const char* executable = nullptr)
        : m_failureCount(0)
        , m_testCount(0)
    ...
    virtual void AddFailure(const char* testName, const char* fileName, int lineNo, const std::string& expected,
                            const std::string& actual)
    {
        printf("\t%s(%i): error: \"Expected: %s, Actual: %s\" in test %s\n", fileName, lineNo, expected.c_str(),
               actual.c_str(), testName);
        m_failureCount++;
    }
...
public:
    ...
    void Run(TestResult& result)
    {
        ...
        for (size_t i = 0; i < m_tests.size(); i++)
        {
            ...
            {
                result.AddFailure(test.TestName_(), test.FileName_(), test.LineNo_(),
...
inline Test::Test(const char* testName, const char* fileName, int lineNo)
    : m_testName_(testName)
...
#define TEST_FIXTURE(testFixture, testName)                                                                            \
    struct testFixture##testName##TestFixture : public testFixture                                                     \
    {                                                                                                                  \
        void Run_(TestResult& result_, const char* m_testName_);                                                       \
    };                                                                                                                 \
    TEST(testFixture, testName)                                                                                        \
    {                                                                                                                  \
        testFixture##testName##TestFixture fixture;                                                                    \
        fixture.Run_(result_, m_testName_);                                                                            \
    }                                                                                                                  \
